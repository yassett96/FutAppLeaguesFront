...
import PopUpTournamentDetailsScoresPlayers from "../../../../components/user/player/tournament/pop-up_tournament_details_scores_players/page";
...
const TablaGoleadores: React.FC<TablaGoleadoresProps> = ({ goleadores, idTorneo, idCategoria }) => {
  ...
  return (
    <div className="bg-transparent shadow border rounded-xl overflow-hidden overflow-x-auto xl1800:w-[97%] ml-[1%] mb-6">
      ...
      <div className="overflow-x-auto overflow-y-auto custom-scrollbar max-h-[415px] sm750:max-h-[615px] h-auto">
        <table className="w-full table-auto">
          <thead className="text-[9px] sm590:text-sm sm670:text-xl bg-[#1e3a8a] text-white h-[60px] sticky top-0 z-10">
            <tr>
              <th className="py-3 px-1 sm500:px-10 text-center font-medium tracking-tight">Jugador</th>
              <th className="py-3 px-1 sm500:px-10 text-center font-medium tracking-tight">Equipo</th>
              <th className="py-3 px-1 sm500:px-10 text-center font-medium tracking-tight">Goles</th>
              <th className="py-3 px-1 sm500:px-10 text-center font-medium tracking-tight">Partidos jugados</th>
              <th className="py-3 px-1 sm500:px-10 text-center font-medium tracking-tight">Acción</th>              
            </tr>
          </thead>
          <tbody className="bg-white">
            {goleadores.map((goleador, index) => (
              <tr key={index} className={`text-xs sm590:text-xl sm670:text-2xl border-b border-gray-200 cursor-pointer ${selectedJugador === goleador.id_jugador ? "bg-blue-200" : ""}`} onClick={() => setSelectedJugador(goleador.id_jugador)}>
                <td className="text-center py-4 px-1 sm500:px-10 text-gray-700">{`${goleador.nombre_jugador} ${goleador.apellido_jugador}`}</td>
                <td className="text-center py-4 px-1 sm500:px-10 text-gray-700">{goleador.equipo}</td>
                <td className="text-center py-4 px-1 sm500:px-10 text-gray-700">{goleador.goles}</td>
                <td className="text-center py-4 px-1 sm500:px-10 text-gray-700">{goleador.partidos_jugados}</td>
                <td className="text-center py-4 px-1 sm500:px-10 text-center flex justify-center">
                  <CustomButton
                    ...
                    className={`flex-col w-full sm750:w-[80%] ${goleador.goles === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                    icon="/images/logos/Icono_Resumen_Blanco.png"
                    classNameIcon="w-6 sm750:w-10 h-6 sm750:h-10"
