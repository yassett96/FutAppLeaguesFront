...
interface TablaSancionadoPendienteProps {
    sancionados: SancionadoPendiente[];
    onEditSanctioned: (sancionado: SancionadoPendiente) => void;
    onDeleteSanctioned: () => void;
    setIsLoading: (isLoading: Boolean) => any;
}

const TablaSancionadoPendiente: React.FC<TablaSancionadoPendienteProps> = ({
    sancionados,
    onEditSanctioned,
    onDeleteSanctioned,
    setIsLoading
        ...
        <div className="bg-transparent shadow border rounded-xl overflow-hidden overflow-x-auto xl1800:w-[97%] ml-[1%] h-auto mb-6">
            <div className="flex items-center px-6 py-4 bg-gray-50 h-[85px]">
                <div className="flex items-center">
                    ...
                    <h2 className="text-xl sm590:text-2xl sm670:text-3xl font-semibold tracking-tight text-gray-700">Sancionados pendientes</h2>
                </div>
            </div>
                    ...
                    <tbody className="bg-white">
                        {sancionados != undefined && sancionados.length > 0 ? (
                            sancionados.map((sancionado, index) => (
                                <tr
                                    key={index}
                                    className={`text-xs sm590:text-xl sm670:text-2xl border-b cursor-pointer ${selectedRow === index ? 'bg-blue-200' : ''}`}
                                ...
                                >
                                    ...
                                    <td className="text-center text-gray-700 flex flex-col items-center justify-center">
                                        <CustomButton
                                            text="Juzgar sancionado"
                                            color="#FFA500"
                                            width=""
                                            height=""
                                            onClick={() => onEditSanctioned(sancionado)}
                                            className="flex-col w-[100px] sm360:w-[180px] sm500:w-[200px] mt-2 mb-2"
                                            classNameText='text-xs sm590:text-xl sm670:text-2xl'
                                            classNameIcon='w-6 sm590:h-8 w-6 sm590:w-8'
                                            icon="/images/logos/Icono_Sancionado_Blanco.png"
                                        />
                                        <CustomButton
                            ...
                            <tr>
                                <td
                                    colSpan={7}
                                    className="text-center py-4 px-6 text-xs sm590:text-base sm670:text-xl text-gray-700"
                                >
...
export default TablaSancionadoPendiente;